...
export const postProveedor = async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO proveedores(Proveedor,Capacidad,Disponible) VALUES (?,?,?)",
      [Proveedor, Capacidad, 1]
    );
  } catch (error) {
    console.log(error);
  }
...
export const changeProveedor = async (req, res) => {
  const { id } = req.params;
  const { Proveedor, Capacidad } = req.body;
  try {
    const [result] = await pool.query(
      "UPDATE proveedores SET Proveedor = ?, CAPACIDAD = ? WHERE id = ? ",
      [Proveedor, Capacidad, id]
    );
